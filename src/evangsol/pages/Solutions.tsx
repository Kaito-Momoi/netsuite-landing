...
const Solutions: React.FC = () => {
  const solutionShowcase = [
    ...
    },
    {
      ...
      description: '複数のECプラットフォームを一つのダッシュボードで完全統合管理。独自のエンジンが売れ筋商品を分析し、在庫配分を自動最適化。',
      points: [
        'Amazon、楽天、Yahoo!、Shopify等の主要ECモール対応',
        '独自のエンジンによる売れ筋予測と在庫自動配分',
    ...
    },
    {
      icon: <BarChart3 />,
      title: '資材計画の見える化',
      subtitle: '独自のエンジン予測で実現する最適調達',
      description: '調達から生産、在庫まで資材の流れをダッシュボードで一元管理。リードタイムと需要予測を考慮した最適発注量を独自のエンジンが自動算出。',
      points: [
        'リアルタイムで資材フローを完全可視化',
        '独自のエンジン予測による最適発注量の自動算出',
        '予防的リスク検知と多段階アラート機能'
      ],
