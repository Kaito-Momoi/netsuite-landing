...
const ProcessSteps: React.FC<ProcessStepsProps> = ({ steps, variant = 'timeline' }) => {
  if (variant === 'grid') {
    return (
          ...
          <div key={index} className="relative">
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center mb-4">
                ...
                {step.duration && (
                  <span className="ml-3 text-sm text-slate-500">{step.duration}</span>
                )}
              </div>
      ...
      {steps.map((step, index) => (
        <div key={index} className="flex items-start space-x-4">
          <div className="flex-shrink-0">
            ...
            {index < steps.length - 1 && <div className="w-0.5 h-20 bg-sky-200 mx-auto mt-2"></div>}
          </div>
          <div className="flex-grow">
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <p className="text-slate-600">{step.description}</p>
              {step.icon && <div className="mt-4 text-sky-600">{step.icon}</div>}
            </div>
          </div>
        </div>
      ))}
