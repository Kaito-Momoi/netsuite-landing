...
const FeatureCard: React.FC<FeatureCardProps> = React.memo(
  ({ feature, size = 'medium', variant = 'default' }) => {
    const sizeClasses = {
      small: 'p-4',
      medium: 'p-6',
      large: 'p-8',
    ...
    return (
      <div className={`rounded-xl ${baseClasses} ${sizeClasses[size]} transition-all`}>
        <div
          className={`${iconSizeClasses[size]} bg-sky-100 rounded-lg flex items-center justify-center mb-4 text-sky-600`}
        >
          {feature.icon}
        </div>
...
export const FeatureGrid: React.FC<FeatureGridProps> = React.memo(
  ({ features, columns = 3, size = 'medium', variant = 'default' }) => {
    const columnClasses = {
      2: 'md:grid-cols-2',
    ...
    return (
      <div className={`grid ${columnClasses[columns]} gap-6`}>
        {features.map((feature) => (
          <FeatureCard key={feature.title} feature={feature} size={size} variant={variant} />
        ))}
      </div>
    );
  }
